...
async function videoToWebp (media) {
    ...
    const tmpFileOut = path.join(tmpdir(), `./tmp/${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.webp`)
    const tmpFileIn = path.join(tmpdir(), `./tmp/${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.mp4`)
...
async function stickerToGif (media) {

    const tmpFileIn = `./tmp/${getRandom(".webp")}`
    const tmpFileOut = `./tmp/${getRandom(".gif")}`

    fs.writeFileSync(tmpFileIn, media)

await cc(tmpFileIn, { animated: true })
    .toFile(tmpFileOut)
    .then(data => console.log(data))
...
/*function tovid(input, output) {
    return new Promise((resolve, reject) => {
        const command = `ffmpeg -i ${input} ${autput}`
...
module.exports = { imageToWebp, videoToWebp, writeExifImg, writeExifVid, writeExif, toImage, stickerToGif, toVideo, /*tovid*/}
